...
func turn(direction):
	if(Direction != 0):
		$Sprites.scale.x = $Sprites.scale.x * -1;
		$PhysicsCollider.scale.x = $PhysicsCollider.scale.x * -1;
...
func phys_execute(delta):
	...
	if(move_states.move):
		...
		if(input_direction != 0 && (true_mspd >= abs(hspd))):
			if(Direction != sign(hspd)):
				true_acceleration = base_acceleration * 1.5;
			else:
				true_acceleration = base_acceleration;
			if(!(true_mspd == abs(hspd)) || sign(hspd) != Direction):
				hspd += true_acceleration * Direction;
			if(true_mspd < abs(hspd)):
				hspd = true_mspd * Direction;
		elif(hspd != 0 && fric_activated):
			if(abs(hspd) <= true_friction):
				hspd = 0;
			else:
				hspd -= true_friction * sign(hspd);
	...
	if(move_states.in_air):
		air_time += delta;
	
	if(on_floor()):
		move_states.in_air = false;
		air_time = 0;
		velocity.y = 0
		vspd = 0;
