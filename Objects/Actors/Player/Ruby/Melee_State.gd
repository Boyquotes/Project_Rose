	...
	if(!attack_triggered):
		...
		if(Input.is_action_just_pressed("dodge")):
			if(host.boosting):
				host.move_states[host.move_state].exit(host.get_node("Movement_States").get_node("Move_In_Air"));
				host.boosting = true;
			if(current_event == 'mattack'):
				current_event += 'dodge';
			else:
				current_event = 'mdodge';
			dashing = true;
			cur_cost = basic_cost;
			attack_triggered = true;
			#TODO: allow input for shortly after start and mid-start for special attacks
		if(Input.is_action_just_pressed("special")):
			host.move_states[host.move_state].exit(host.get_node("Movement_States").get_node("Boost"));
			host.boosting = true;
		if(Input.is_action_just_pressed("switch")):
			exit(host.get_node("Style_States").get_node("Ranged"));
	if(attack_triggered):
		busy = true;
		set_position_vars();
	pass;
