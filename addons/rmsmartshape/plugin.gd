...
# Data related to an action being taken checked points
...
func _draw_control_point_line(c: Control, vert: Vector2, cp: Vector2, tex: Texture2D):
	# Draw the line with a dark and light color to be visible checked all backgrounds
...
func _input_handle_mouse_button_event(
	...
	#########################################
	# Mouse Wheel checked valid point
...
func get_mouse_over_vert_key(mm: InputEventMouseMotion, grab_threshold: float) -> int:
	var xform: Transform2D = get_et() * shape.get_global_transform()
	# However, if near a control point or one of its handles then we are not checked the edge
...
func _input_handle_mouse_motion_event(
	...
	gui_point_info_panel.position = mm.position + Vector2(256, -24)
